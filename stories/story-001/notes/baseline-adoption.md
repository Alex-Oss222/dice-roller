...
The original source inputs remain provenance. Ordinary play must not rewrite shared pins. Any later shared-contract change requires another explicit compatibility review before new story mutations are accepted.
...
## 23 September 2026 shared-layer upgrade after research

The player authorized the shared changes planned from the LLM-chatbot research report (reports/) and reviewed audit: paced advancement and opposition records at setup in the rules; an example page and a person-record template; engine guards (narrative lint, a named ability for death, a lowered Condition or a divergence, one rating step per event, canonical journey keys); reader-facing renderer formats and grouped correction notes; and repository hooks that protect generated pages and validate before a turn ends.

Compatibility review: Story 1 has three accepted events (setup, the sheet correction of 22 September and the wording correction of 23 September) and no resolved turn. None carries an advance payload, a journey record, a training period or a divergence, so the new engine checks replay them unchanged and no accepted fact, rating, time, location or opening prose is affected. Reading views are regenerated by the engine from the same events. Story 1 adopts the reviewed shared hashes in story.json; ordinary play must not re-pin again without another reviewed change. No opposition person record is added here: those belong to the story's own preparation before Turn 1 and are not copied from any other story.
