...
`adjudication` has the base fields `{mode, actor, capability, preparation, opposition, risk, basis, task_band}` and may also contain `supporting_capabilities`. Mode is `routine` or `uncertain`; actor is `pc` or an existing `person` record ID. Preparation, opposition, risk, and basis are nonempty explanations. Routine mode uses task band `routine`. Uncertain mode uses `ordinary`, `demanding`, `hard`, or `extreme` and requires an already established primary `capability`. A primary capability is `{source, key}`. Each supporting capability is `{source, key, role}`, where `role` states the concrete part of execution that ability governs. PC sources are `character.skills`; NPC sources are `world.records.ID.details` for that same actor. Every key must exist before resolution, and the same capability cannot be duplicated as both primary and supporting or repeated in the support list. Routine mode permits null primary capability. Older accepted workflow events without `supporting_capabilities` replay as an empty list. In a Blood & Gold campaign, PC adjudication references must identify a specific sub-skill, specialty, or derived ability; a broad domain is rejected as a substitute for an action skill. Supporting ratings are not summed, averaged, or treated as bonuses to the primary ability. Code establishes that referenced evidence exists, not that the GM selected every relevant ability or explained its causal role honestly.
