...
The [book reference index](../references/books.md), source policy, and distance catalog are shared across independent stories. Begin by reading the selected story's character sheet and source limits. Store its researched rulings, assumptions, inventions, and divergences under `stories/<id>/`, including canonical research events where used. A Story 1 ruling cannot alter shared book canon or become Story 2 knowledge. Shared reference corrections are separately reviewed maintenance, never a way to make a particular story's outcome convenient.
