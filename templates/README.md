...
The GM writes the scene once in `narrative`; the engine builds the story's reading pages and current records. The model does not need to reproduce an entire sheet for every small update. The GM verifies publication and sends the player the reading link. See [the workflow](../docs/play_workflow.md).
