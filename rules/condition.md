...
Use the recorded number, label/band where useful, and tags in the small turn header or a changed Ledger entry. Keep numeric Condition out of scene narration and never turn it into a Capability bonus. Narrate concrete effects instead. If a legacy state has no summary, show `Not established` rather than deriving a number without an accepted basis.
