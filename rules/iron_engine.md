...
The story is written in chat and saved as each accepted turn's narrative. Inside the selected story folder, generate `play/story.md` from those exact records and `play/character-sheet.md` plus `play/resume.md` from current state. For example, story-001's journal is `stories/story-001/play/story.md`; its root `character-sheet.md` remains the starting input. These readable files are regenerated views with a source hash, not separately editable campaign truth. Resume from validated records, preserving accepted wording and unresolved choices. An error is explained through a correction rather than silently rewriting a past scene.
