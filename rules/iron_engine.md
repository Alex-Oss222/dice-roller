...
Show a Ledger only when material persistent facts changed, and show only those changes. Apply every listed change to the accepted state. If no material sheet/world fact changed, omit the Ledger entirely. Display an actual next decision only when it interrupts further authorized activity. Do not supply suggested actions, objectives, plans, or menus unless requested, and do not invent a cliffhanger to fill the format.
