...
Use the narrowest established sub-skill, specialty, or justified derived ability that actually governs the attempted task. A broad domain is only descriptive for genuinely broad work and, in Blood & Gold workflow adjudication, is not a substitute for the specific action skill. When several abilities materially matter, choose one primary capability for the main uncertainty and record the others as supporting capabilities with explicit roles. Each ability governs its own part of execution rather than contributing a pooled or averaged score. For example, a weapon skill governs weapon handling, Battlefield Awareness governs situational perception, Armored Fighting governs movement and recovery in harness, Tactics governs a local battlefield problem, and a command specialty governs issuing or coordinating orders when applicable. Include only abilities the actual action uses. Consider equipment, physical attributes, wounds, fatigue, numbers, information, preparation, position, opposition, and resources separately. Do not add a domain and its contributing sub-skill as two advantages. Routine feasible work succeeds directly; impossibility needs no check. Briefly identify the decisive cause for consequential outcomes.
